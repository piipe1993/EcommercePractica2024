...
public class FuncionamientoPrimerCarruselTask implements Task {
        ...
        public static List<WebElementFacade> ProductosPrimerCarrusel,primerCarrusel = new ArrayList<>();

    public FuncionamientoPrimerCarruselTask(String SKU) {
        this.SKU = SKU;
    }
    ...
    @Override
        public <T extends Actor > void performAs (T actor){
            ...
            actor.attemptsTo(WaitUntil.the(BTN_SIGUIENTE_PRODUCTO_PRIMER_CARRUSEL, isClickable()).forNoMoreThan(30).seconds());
            actor.attemptsTo(Scroll.to(BTN_SIGUIENTE_PRODUCTO_PRIMER_CARRUSEL).andAlignToBottom());
            JavascriptExecutor jscriptExecutor = (JavascriptExecutor) ThucydidesWebDriverSupport.getDriver();
            PaginaStateRedy.completed();
            UtilsWaits.wait(5);
            ProductosPrimerCarrusel = BTNS_PRIMER_CARRUSEL.resolveAllFor(actor);
            //Inicio Validacion Productos Carrusel- Martha De la Ossa
            primerCarrusel=PRIMER_CARRUSEL.resolveAllFor(actor);
            if(primerCarrusel.size()>1){
                Serenity.recordReportData().withTitle("Existen "+primerCarrusel.size()+" carruseles en esta seccion: MasPopulares/Mas Vendidos, por favor revisar" )
                        .andContents("Existen "+primerCarrusel.size()+" carruseles en esta seccion: MasPopulares/Mas Vendidos, por favor revisar" );
                Assert.fail ("Existen "+primerCarrusel.size()+" carruseles en esta seccion: MasPopulares/Mas Vendidos, por favor revisar");

            }

            for (int i = 0; i < (ProductosPrimerCarrusel.size()); i++) {
                boolean clickProducto = false;
                int secure=0;
                while ((!clickProducto)) {
                    secure=secure+1;
                    actor.attemptsTo(
                           MoveMouse.to(BTN_SIGUIENTE_PRODUCTO_PRIMER_CARRUSEL)
                    );

                    if (BTN_INDEX_ACTIVO_PRIMER_CARRUSEL(i).resolveFor(actor).isClickable()) {
                        nombreProductoActivo= (LBL_NOMBRE_PRODUCTO_ACTIVO_PRIMER_CARRUSEL.resolveFor(actor).getText()).trim();
                        SKUProductoActivo=(LBL_SKU_PRODUCTO_ACTIVO_PRIMER_CARRUSEL.resolveFor(actor).getText()).replaceAll("SKU:","").trim();
                        if (listSKU.contains(SKUProductoActivo)){
                            listSKU.remove(listSKU.indexOf(SKUProductoActivo));
                        }
                        actor.attemptsTo(Click.on (BTN_INDEX_ACTIVO_PRIMER_CARRUSEL(i)));
                        clickProducto = true;
                        secure=0;
                    }else{
                        actor.attemptsTo(
                                Click.on (BTN_SIGUIENTE_PRODUCTO_PRIMER_CARRUSEL),
                                MoveMouse.to(BTN_SIGUIENTE_PRODUCTO_SEGUNDO_CARRUSEL)
                        );
                    }

                    if (secure>((ProductosPrimerCarrusel.size())*4)){
                        ...
                        Assert.fail ("Existe un problemas con los botones de atras y/o adelante del 1 carrusel por favor revisar");
                ...
                Serenity.takeScreenshot();
                actor.attemptsTo(
                        WaitUntil.the(PaginaInicioUI.BTN_SIGUIENTE_PRODUCTO_PRIMER_CARRUSEL, isClickable()).forNoMoreThan(30).seconds()
                );
            }
    ...
    public static Performable ir (String SKU) {
        return Instrumented.instanceOf(FuncionamientoPrimerCarruselTask.class).withProperties(SKU);
    }
}

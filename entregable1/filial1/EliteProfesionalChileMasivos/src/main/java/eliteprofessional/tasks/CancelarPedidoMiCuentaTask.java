...
public class CancelarPedidoMiCuentaTask implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        if (!BTN_SOLICITAR_CANCELACION.isVisibleFor(actor)) {
            Serenity.recordReportData().withTitle(" El pedido no se tiene opcion de Cancelar verifique si fue cancelado antes")
                    .andContents("El pedido no se tiene opcion de Cancelar verifique si fue cancelado antes");
            Assert.fail("El pedido no se tiene opcion de Cancelar verifique si fue cancelado antes");
        }
